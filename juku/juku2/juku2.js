function juku2() {
    ...
    function setup() {
        ...
        switch (p) {
            case 0:
                ...
                n3 = getRandom(2, 3);
                a = n1 - n2 * n3;
                $mondai.innerText = n1 + `円玉1まいで` + n2 + `円のおかしを` + n3 + `こ買いました。おつりは何円?`;
                break;
            case 1:
                n1 = 500;
                n2 = getRandom(4, 6) * 10;
                n3 = getRandom(3, 8);
                a = n1 - n2 * n3;
                $mondai.innerText = n1 + `円玉1まいで` + n2 + `円のえんぴつを` + n3 + `本買いました。おつりは何円?`;
                break;
            case 2:
                n1 = 1000;
                n2 = getRandom(2, 3) * 100;
                n3 = getRandom(2, 3);
                a = n1 - n2 * n3;
                $mondai.innerText = `千円さつ1まいで` + n2 + `円のノートを` + n3 + `さつ買いました。おつりは何円?`;
                break;
            default:
                alert(`リロードして下さい`);
        };
    ...
    function checkAnswer() {
        ...
        if ($kotae.value == a) {
            ...
            mistakes.push($mondai.innerText + `　＜正解：` + a + `円＞`);
        };
        count++;
        if (count < countMax) {
            setup();
